...
import {
  ...
  ResponsiveContainer, Tooltip, Line, ComposedChart, Area,
} from 'recharts';
import { ArrowLeft } from 'lucide-react';

interface MonthlyTrendContentProps {
...
export function MonthlyTrendContent({ contracts, categories }: MonthlyTrendContentProps) {
  ...
  const timeBounds = useMemo(() => {
    ...
    const ends = contracts.filter((c) => c.end_date).map((c) => new Date(c.end_date!));
  ...
  // Compute monthly trend data (non-cumulative)
  const chartData = useMemo(() => {
    ...
    while (cursor < toDate) {
      ...
      monthlyPoints.push({ date: new Date(cursor), values: monthValues });
      cursor.setMonth(cursor.getMonth() + 1);
    }

    // Apply granularity
    if (granularity === 'yearly') {
      const yearMap = new Map<string, { date: Date; total: Record<string, number>; count: number }>();
      for (const point of monthlyPoints) {
        const year = String(point.date.getFullYear());
        const existing = yearMap.get(year);
        if (existing) {
          for (const [catId, amount] of Object.entries(point.values)) {
            existing.total[catId] = (existing.total[catId] ?? 0) + amount;
          }
          existing.count++;
        } else {
          yearMap.set(year, { date: point.date, total: { ...point.values }, count: 1 });
        }
      }
      return Array.from(yearMap.values()).map((point) => {
        ...
        for (const catId of activeCategoryIds) {
          if (enabledCategories.has(catId)) {
            const val = Math.round((point.total[catId] ?? 0) * 100) / 100;
  ...
  }, [contracts, activeCategoryIds, enabledCategories, granularity, rangeFrom, rangeTo, categoryMap, showGross]);

  // Running average: sum of all totals up to that point / number of periods seen
  const chartDataWithAverage = useMemo(() => {
    if (!showAverage) return chartData;
    let runningSum = 0;
    return chartData.map((d, idx) => {
      runningSum += Number(d._total ?? 0);
      return {
        ...d,
        _average: Math.round((runningSum / (idx + 1)) * 100) / 100,
      };
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{t('monthlyTrend')}</h1>
      </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    enabled ? 'border-transparent text-white' : 'border-zinc-300 text-zinc-400 dark:border-zinc-700'
                  )}
                ...
                <ComposedChart data={chartDataWithAverage} margin={{ top: 10, right: 30, bottom: 5, left: 20 }}>
                  <defs>
                    {enabledCategoryList.map((cat) => (
                      <linearGradient key={cat.id} id={`trend-grad-${cat.id}`} x1="0" y1="0" x2="0" y2="1">
